...
pub fn registerDomApis(rt: *qjs.JSRuntime, ctx: *qjs.JSContext, document_ptr: *anyopaque) void {
    ...
    // Embedded/Media/Form/Table/Section element IDL attribute reflection
    {
        const embed_js =
            ...
            \\  if(typeof HTMLFormElement!=='undefined'){var FM=HTMLFormElement;rs(FM,'name');re(FM,'method',null,['get','post','dialog'],'get','get');rs(FM,'target');rs(FM,'acceptCharset','accept-charset');ru(FM,'action');re(FM,'enctype',null,['application/x-www-form-urlencoded','multipart/form-data','text/plain'],'application/x-www-form-urlencoded','application/x-www-form-urlencoded');re(FM,'encoding','enctype',['application/x-www-form-urlencoded','multipart/form-data','text/plain'],'application/x-www-form-urlencoded','application/x-www-form-urlencoded');rs(FM,'autocomplete');rs(FM,'rel');rb(FM,'noValidate','novalidate');}
            ...
            \\  if(typeof HTMLInputElement!=='undefined'){var IN=HTMLInputElement;rs(IN,'name');re(IN,'type',null,['hidden','text','search','tel','url','email','password','date','month','week','time','datetime-local','number','range','color','checkbox','radio','file','submit','image','reset','button'],'text','text');rs(IN,'value');rs(IN,'defaultValue','value');rs(IN,'accept');rs(IN,'alt');rs(IN,'autocomplete');rs(IN,'dirName','dirname');ru(IN,'formAction','formaction');re(IN,'formEnctype','formenctype',['application/x-www-form-urlencoded','multipart/form-data','text/plain'],'','application/x-www-form-urlencoded');re(IN,'formMethod','formmethod',['get','post','dialog'],'','get');rs(IN,'formTarget','formtarget');rs(IN,'max');rs(IN,'min');rs(IN,'pattern');rs(IN,'placeholder');ru(IN,'src');rs(IN,'step');rs(IN,'align');rs(IN,'useMap','usemap');rs(IN,'capture');rs(IN,'popoverTargetAction','popovertargetaction');ri(IN,'width');ri(IN,'height');ri(IN,'maxLength','maxlength',-1);ri(IN,'minLength','minlength',-1);riu(IN,'size',null,20,1);rb(IN,'disabled');rb(IN,'checked');rb(IN,'defaultChecked','checked');rb(IN,'multiple');rb(IN,'readOnly','readonly');rb(IN,'required');rb(IN,'formNoValidate','formnovalidate');rb(IN,'autofocus');rb(IN,'indeterminate');}
            \\  if(typeof HTMLButtonElement!=='undefined'){var BT=HTMLButtonElement;rs(BT,'name');re(BT,'type',null,['submit','reset','button'],'submit','submit');rs(BT,'value');ru(BT,'formAction','formaction');re(BT,'formEnctype','formenctype',['application/x-www-form-urlencoded','multipart/form-data','text/plain'],'','application/x-www-form-urlencoded');re(BT,'formMethod','formmethod',['get','post','dialog'],'','get');rs(BT,'formTarget','formtarget');rs(BT,'popoverTargetAction','popovertargetaction');rb(BT,'disabled');rb(BT,'autofocus');rb(BT,'formNoValidate','formnovalidate');}
