...
fn syncDeclListToAttr(elem: *lxb.lxb_dom_element_t, list: *style_decl_mod.StyleDeclList) void {
    ...
    defer serialized.deinit(std.heap.c_allocator);
...
pub fn registerDomApis(rt: *qjs.JSRuntime, ctx: *qjs.JSContext, document_ptr: *anyopaque) void {
    ...
    _ = qjs.JS_SetPropertyStr(ctx, node_proto, "__beginMutSup", qjs.JS_NewCFunction(ctx, &dom_node.jsBeginMutSuppress, "__beginMutSup", 0));
    _ = qjs.JS_SetPropertyStr(ctx, node_proto, "__endMutSup", qjs.JS_NewCFunction(ctx, &dom_node.jsEndMutSuppress, "__endMutSup", 0));
    ...
    // Embedded/Media/Form/Table/Section element IDL attribute reflection
    {
        const embed_js =
            ...
            \\  function riu(C,p,a,d,min){if(!a)a=p.toLowerCase();if(d===undefined)d=0;if(min===undefined)min=0;Object.defineProperty(C.prototype,p,{get:function(){var s=this.getAttribute(a);if(s===null)return d;var v=_pint(s);if(isNaN(v)||v<min||v<0||v>2147483647)return d;return v;},set:function(v){v=v>>>0;if(v>2147483647)v=d;if(min>0&&v<min)throw new DOMException('Index or size is negative or greater than the allowed amount','IndexSizeError');this.setAttribute(a,''+v);},configurable:true,enumerable:true});}
            \\  function ris(C,p,a,d,min){if(!a)a=p.toLowerCase();if(d===undefined)d=0;if(min===undefined)min=-2147483648;Object.defineProperty(C.prototype,p,{get:function(){var s=this.getAttribute(a);if(s===null)return d;var v=_pint(s);if(isNaN(v)||v<min||v>2147483647)return d;return v===0&&(1/v)===-Infinity?0:v;},set:function(v){v=+v;if(v<min)throw new DOMException('Index or size is negative or greater than the allowed amount','IndexSizeError');this.setAttribute(a,''+v);},configurable:true,enumerable:true});}
    ...
    // Document constructor (new Document() creates a standalone XML document-like object)
    {
        const doc_ctor_js =
            ...
            \\  Document.prototype.createElement = function(t) { var e=document.createElement(t); _setOwnerDoc(e,this);if(this._isXmlDoc){_defProp(e,'namespaceURI',null);_defProp(e,'tagName',t);_defProp(e,'localName',t);_defProp(e,'nodeName',t);}return e; };
            ...
            \\  Document.prototype.createCDATASection = function(d) { var n={nodeType:4,nodeName:'#cdata-section',data:d,textContent:d,nodeValue:d,childNodes:[],parentNode:null,previousSibling:null,nextSibling:null,firstChild:null,lastChild:null,ownerDocument:this};if(typeof Text!=='undefined'&&Text.prototype)Object.setPrototypeOf(n,Text.prototype);return n; };
    ...
    // Selector validation shim — throws TypeError/SyntaxError per DOM spec
    {
        const sel_valid_js =
            ...
            \\  Element.prototype.closest=function(s){if(arguments.length<1)throw new TypeError("Failed to execute 'closest': 1 argument required, but only 0 present.");_vSel(s,'closest');return _nc.call(this,_autoClose(String(s)));};
            \\  Element.prototype.querySelector=function(s){if(arguments.length<1)throw new TypeError("Failed to execute 'querySelector': 1 argument required, but only 0 present.");_vSel(s,'querySelector');return _nqs.call(this,_autoClose(String(s)));};
            \\  Element.prototype.querySelectorAll=function(s){if(arguments.length<1)throw new TypeError("Failed to execute 'querySelectorAll': 1 argument required, but only 0 present.");_vSel(s,'querySelectorAll');return _nqsa.call(this,_autoClose(String(s)));};
            \\  var _dqs=document.querySelector,_dqsa=document.querySelectorAll;
            \\  document.querySelector=function(s){_vSel(s,'querySelector');return _dqs.call(document,_autoClose(String(s)));};
            \\  document.querySelectorAll=function(s){_vSel(s,'querySelectorAll');return _dqsa.call(document,_autoClose(String(s)));};
            \\})()
        ;
    ...
    // ── CSSOM: CSSStyleSheet, CSSStyleRule, style.sheet, CSS.supports("selector()") ──
    {
        const cssom_js =
            ...
            \\        i=j;
            \\      }else if(si!==-1){
            \\        var d3=body.substring(i,si+1).trim();
            \\        if(d3){if(children.length===0)decls+=(decls?' ':'')+d3;else pendingDecls+=(pendingDecls?' ':'')+d3;}
            \\        i=si+1;
            \\      }else{
            \\        var d4=body.substring(i).trim();
            \\        if(d4){if(children.length===0)decls+=(decls?' ':'')+d4;else pendingDecls+=(pendingDecls?' ':'')+d4;}
            \\        break;
            \\      }
            \\    }
