...
fn documentCreateRange(
    ...
    _ = qjs.JS_SetPropertyStr(c, range, "getBoundingClientRect", qjs.JS_NewCFunction(c, &dom_elem.elementGetBoundingClientRect, "getBoundingClientRect", 0));
    return range;
}
...
fn documentGetElementById(
    ...
    const found = dom_sel.walkTreeById(doc_node, s.ptr[0..s.len]) orelse return quickjs.JS_NULL();
...
pub fn walkTreeByTagAndClass(root: *lxb.lxb_dom_node_t, tag_name: []const u8, class_name: []const u8) ?*lxb.lxb_dom_node_t {
    ...
    while (current) |node| {
        ...
        current = dom_sel.nextDfsNode(node, root);
    }
    return null;
}
...
fn documentGetElementsByClassName(
    ...
    var idx: u32 = 0;
    dom_sel.walkTreeCollect(c, doc_node, selector_buf[0 .. 1 + class_name.len], arr, &idx);
    return arr;
}
...
fn documentGetElementsByTagName(
    ...
    const doc_node = getDocumentNode() orelse return arr;
    var idx: u32 = 0;
    dom_sel.walkTreeCollect(c, doc_node, s.ptr[0..s.len], arr, &idx);
    return arr;
}
...
fn documentGetElementsByName(
    ...
    const doc_node = getDocumentNode() orelse return arr;
    var idx: u32 = 0;
    dom_sel.walkTreeCollect(c, doc_node, selector_buf[0 .. prefix.len + name.len + suffix.len], arr, &idx);
    return arr;
}
