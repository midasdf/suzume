...
pub fn registerWebApis(js_rt: anytype) void {
    ...
    const compat_stubs =
        ...
        \\      xmlDoc.createElement=(type==='application/xhtml+xml')?function(t){return document.createElement(t);}:function(t){var e=document.createElement(t);Object.defineProperty(e,'namespaceURI',{value:null,configurable:true});Object.defineProperty(e,'__origLocal',{value:String(t),writable:true});e.__xmlCaseSensitive=true;return e;};
        ...
        \\    for(var i=0;i<signals.length;i++){(function(sig){sig.addEventListener('abort',function(){if(!s.aborted){s.aborted=true;s.reason=sig.reason;var e=new Event('abort');e._trusted=true;e.target=s;e.currentTarget=s;if(s.onabort)try{s.onabort(e);}catch(ex){}s.dispatchEvent(e);}});})(signals[i]);}return s;};
        ...
        \\  BroadcastChannel.prototype.addEventListener=function(t,f){if(t==='message')this.onmessage=f;};
        \\  BroadcastChannel.prototype.removeEventListener=function(){};
        \\}
        \\if(typeof PerformanceObserver==='undefined'){
        \\  globalThis.PerformanceObserver=function(cb){this._cb=cb;};
        \\  PerformanceObserver.prototype.observe=function(){};PerformanceObserver.prototype.disconnect=function(){};
        \\  PerformanceObserver.prototype.takeRecords=function(){return[];};
        \\  PerformanceObserver.supportedEntryTypes=[];
        \\}
        \\if(typeof ReadableStream==='undefined'){
        \\  globalThis.ReadableStream=function(src,strategy){this._src=src;this._queue=[];this._closed=false;this._controller={enqueue:function(chunk){this._rs._queue.push(chunk);}.bind({_rs:this}),close:function(){this._rs._closed=true;}.bind({_rs:this}),error:function(){}};if(src&&src.start)src.start(this._controller);};
        \\  ReadableStream.prototype.getReader=function(){var rs=this;return{read:function(){if(rs._queue.length>0)return Promise.resolve({value:rs._queue.shift(),done:false});if(rs._closed)return Promise.resolve({value:undefined,done:true});return new Promise(function(resolve){var check=function(){if(rs._queue.length>0)resolve({value:rs._queue.shift(),done:false});else if(rs._closed)resolve({value:undefined,done:true});else setTimeout(check,10);};check();});},releaseLock:function(){},cancel:function(){rs._closed=true;return Promise.resolve();}};};
        \\  ReadableStream.prototype.cancel=function(){this._closed=true;return Promise.resolve();};
        \\  ReadableStream.prototype[Symbol.asyncIterator]=function(){var r=this.getReader();return{next:function(){return r.read();}};};
        \\}
        \\if(typeof WritableStream==='undefined'){
        \\  globalThis.WritableStream=function(sink){this._sink=sink||{};};
        \\  WritableStream.prototype.getWriter=function(){var ws=this;return{write:function(chunk){if(ws._sink.write)return Promise.resolve(ws._sink.write(chunk));return Promise.resolve();},close:function(){if(ws._sink.close)ws._sink.close();return Promise.resolve();},releaseLock:function(){},abort:function(){return Promise.resolve();}};};
        \\}
        \\if(typeof TransformStream==='undefined'){
        \\  globalThis.TransformStream=function(transformer){
        \\    var chunks=[];var rc={enqueue:function(c){chunks.push(c);}};
        \\    this.readable=new ReadableStream({start:function(ctrl){},pull:function(){if(chunks.length)ctrl.enqueue(chunks.shift());}});
        \\    this.writable=new WritableStream({write:function(chunk){if(transformer&&transformer.transform)transformer.transform(chunk,rc);else rc.enqueue(chunk);}});
        \\  };
        \\}
